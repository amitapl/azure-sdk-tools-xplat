...
ScriptGenerator.prototype.generateWapDeploymentScript = function (_) {
  ...
  var msbuildArguments = '"%DEPLOYMENT_SOURCE%\\' + this.projectPath + '" /nologo /verbosity:m /t:Build /t:pipelinePreDeployCopyAllFilesToOneFolder /p:_PackageTempDir="%DEPLOYMENT_TEMP%";AutoParameterizationWebConfigConnectionStrings=false;Configuration=Release %SCM_BUILD_ARGS%';

  if (this.solutionPath != null) {
    var solutionDir = path.dirname(this.solutionPath);
    msbuildArguments += ' /p:SolutionDir="%DEPLOYMENT_SOURCE%\\' + solutionDir + '\\\\\" %SCM_BUILD_ARGS%';
  }
